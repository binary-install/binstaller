...
Official installers generated by [actionutils/trusted-go-releaser](https://github.com/actionutils/trusted-go-releaser) release workflow, which uses binstaller itself to create these secure installers.

```mermaid
graph TB
    A[trusted-go-releaser workflow] -->|builds| B[binstaller binary]
    A -->|generates checksums| C[SHA256 checksums]
    B -->|verified by| C
    
    A -->|uses binstaller| D["binst embed-checksums<br/>(embeds verified checksums)"]
    D -->|generates| E["binst gen --target-version<br/>(creates installer script)"]
    
    E -->|produces| F[install.sh with<br/>embedded checksums]
    
    A -->|creates attestation for| B
    A -->|creates attestation for| F
    
    F -->|user verifies| G[Attestation Check]
    G -->|ensures| H[✓ Authentic checksums<br/>✓ Trusted installer<br/>✓ Binary matches checksums]
...
**Key benefit**: When you verify the installer script's attestation, you automatically verify the entire chain - no additional verification needed after installation.
